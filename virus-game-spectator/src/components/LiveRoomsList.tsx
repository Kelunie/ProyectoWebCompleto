...
function extractRooms(data: any): RoomSummary[] {
      ...
      <header className="border-b border-white/5 py-6 px-8">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-2xl font-bold bg-gradient-to-r from-violet-400 to-cyan-300 bg-clip-text text-transparent">
              Virus Game
            </h1>
          </div>
      ...
      <main className="flex-1 max-w-6xl mx-auto w-full px-8 py-10">
        ...
        <div className="flex gap-2 mb-6">
          {(['all', 'live', 'open'] as const).map(tab => (
            ...
            >
              {tab === 'all' ? 'Todos' : tab === 'live' ? 'En vivo' : 'Pendientes'}
            </button>
          ))}
        </div>
        ...
        {!loading && !error && filteredRooms.length === 0 && (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <h3 className="text-xl font-medium text-white mb-2">No hay salas</h3>
