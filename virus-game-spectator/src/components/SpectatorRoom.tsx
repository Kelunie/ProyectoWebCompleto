...
import type { RoomActionLog, Player } from "../types/game";
...
function renderActionText(action: RoomActionLog, players: Player[]) {
  ...
  const target =
    action.target_name || playerNameById(players, action.target_id);
  switch (action.type) {
    case "vote":
      return `${actor} votó por ${target}`;
    case "terror_infect":
      return `${actor} intentó infectar a ${target}`;
    case "investigate":
      return `${actor} investigó a ${target}`;
    case "advance_phase":
      return `${actor} avanzó la fase`;
    case "start_game":
      return `${actor} inició la partida`;
    default:
      return action.message || `${actor} realizó ${action.type}`;
  }
}

export default function SpectatorRoom() {
  const { roomId } = useParams<{ roomId: string }>();
  const { state, chatLog, actionsLog, loading, syncing, error } =
    useSpectatorRoom(roomId!);
  ...
  const infected = (state.infectedPlayerIds || []).map((id) =>
    playerNameById(state.players, id),
  );
  const hostName = playerNameById(state.players, state.hostId);
  const votingCountdown = state.voting_remaining_seconds ?? 0;
              ...
              {state.players.map((p) => (
                <div key={p.id} className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div
                      className={`w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-bold ${p.alive ? "bg-emerald-500" : "bg-slate-600"}`}
                    >
                  ...
                  <span
                    className={`px-2 py-0.5 rounded-full text-xs ${p.alive ? "bg-emerald-400/10 text-emerald-400" : "bg-slate-700 text-slate-400"}`}
                  >
                    {p.alive ? "Vivo" : "Muerto"}
                  </span>
                </div>
                    ...
                    >
                      ...
                      <div>
                        <p className="text-slate-300">
                          {renderActionText(action, state.players)}
                        </p>
        ...
        {state.ended && (
          <div className="mt-8 glass rounded-2xl p-6 text-center border border-yellow-400/20">
            ...
            <p className="text-white">
              Ganador: {state.winners[0] || "Sin definir"}
            </p>
          </div>
        )}
      </main>
